...
# 
#######################################################################################
# Download the MovieLens 10M dataset: http://files.grouplens.org/datasets/movielens/ml-10m.zip
...
edx_counts <- edx %>% summarize (ratings=nrow(.),     # determine number of ratings
...
save(edx_str, file="rdas/edx_str.rda")  # save for loading into rmarkdown
save(edx_counts, file="rdas/edx_counts.rda")  # save for loading into rmarkdown
...
movie_counts<- edx %>%
  ...
  theme(plot.title =element_text(size=16, face ="bold" ), # change title font size
...
# Determine if average ratings are uniform or if some movies have higher averages
movie_averages <- edx %>%
  group_by(movieId) %>%
  ...
  theme(plot.title =element_text(size=16, face ="bold" ), #change font size of all text
        axis.text=element_text(size=14), #change font size of axis text
        axis.title=element_text(size=16)) #change font size of axis titles
...
edx<- edx %>%
  separate(genres, sep = "\\|",into=c("genre1", "genre2"), extra="merge", 
...
genre_counts <- edx %>%
  ...
  xlab("number of ratings") + ylab("movie genres") + 
  theme(plot.title =element_text(size=16, face ="bold" ), # change title size
        axis.text=element_text(size=14), #change font size of axis text
        axis.title=element_text(size=16)) #change font size of axis titles
...
genre_averages <- edx %>%
  group_by(genre1) %>%
  filter(n()>100)  %>%
  summarize(n=n(), avg=mean(rating), se=sd(rating)/sqrt(n())) %>%  # mean & standard error 
  ...
  xlab("movie genres") + ylab("average rating") + 
  theme(plot.title =element_text(size=16, face ="bold" ), # change title size
        axis.text=element_text(size=14), #change font size of axis text
        axis.title=element_text(size=16)) #change font size of axis titles
...
time_week<- edx %>%
  ...
  theme(plot.title =element_text(size=16, face ="bold" ), #change size of text
        axis.text=element_text(size=14), #change font size of axis text
        axis.title=element_text(size=16)) #change font size of axis titles
...
time_weekday <- edx %>%
  ...
  coord_flip() +
  theme_gray (base_size= 20)+
  ggtitle("Average Rating by Week Day") +
...
# GROUP PATTERNS
#---------------------------------------------------------------------------------
# Determine if there are groups of movies or users with similar ratings patterns.
...
names(groups)[groups==3]  # family 
names(groups)[groups==4]   
names(groups)[groups==5]  # blockbusters
names(groups)[groups==6] 
names(groups)[groups==7]  # heroes
names(groups)[groups==8]  
...
#------------------------------------------------------------------------------------------
# Start with basic model that predicts same value mu for all unknown ratings: Yui= mu + eui
...
RMSE<- sqrt(mean((test_set$rating - mu)^2))   # RMSE of predicting mu for unknown ratings
...
#------------------------------------------------------------------------------------------
# Augment model to adjust for variability bi in movie-to-movie ratings: Yui= mu + bi + eui
...
#--------------------------------------------------------------------------------------
# Improve model by "penalizing" large estimates formed using movies or users with 
# small number of ratings
# Select the regularization parameter lambda that optimizes RMSE for movie & user effects:
...
#-------------------------------------------------------------------------------------
# Augment model to adjust for variability in genre to genre gui ratings:   
# Yui= mu + reg_bi + reg_bu + gui+ eui,

# gui(s) are the average difference in ratings by genre after removing 
...
# predictions for unknown ratings in the test_set using: Yui= mu + reg_bi + reg_bu + gui
predictions<- test_set %>%
  left_join(reg_bis, by="movieId") %>%
  left_join(reg_bus, by="userId") %>%
...
#------------------------------------------------------------------------------------
# Adjust for variability of ratings by time  dui:  
# Yui = mu + reg_bi + reg_bu  + gui + dui + eui

# dui(s) are the average difference in ratings by time, after removing user, 
# movie & genre effects
...
###################################################################
#
# FINAL TEST of EFFECTS MODEL on VALIDATION SET
#
##################################################################

# Use full edx set and recalculate reg_bis, reg_bus, guis and duis, with optimal lambda
...
# predictions on the validation set using the final model: 
# Yui = mu + reg_bi + reg_bu  + gui + dui
...
RMSE<- sqrt(mean((validation$rating - predictions)^2))  # RMSE of the predictions
results$MODEL[8]<-"VALIDATION test: Effects model"
results$RMSE[8] <- round(RMSE,5)


############################################################################################
#
#  MODELING WITH JUST RECOMMMENDERLAB
#
###########################################################################################
# We have observed that there are groups of users and movies with similar rating patterns

# Use Recommenderlab's implementation of matrix factorization/ SVD for predicting ratings 
# based on proximity of users and movies. See: 
# https://cran.r-project.org/web/packages/recommenderlab/vignettes/recommenderlab.pdf

# transform edx dataframe into a sparse matrix which is more efficient storing sparse data
...
# use Recommenderlab to partition data into training, test and error sets. 
frequent_users_split <- evaluationScheme(frequent_users, method="split", 
                                         train= 0.9, given=30, goodRating=5)
...
###############################################################################################
#
#  COMBINED MODEL: RECOMMENDERLAB + EFFECTS MODEL's RESIDUALS
#
##############################################################################################

# Let's try to improve estimates by fitting: Yui = mu + reg_bi + reg_bu+ gui + dui + rui + eui
# with rui the residuals after adjusting for all previous effects:  
# res= rating- (mu + reg_bi + reg_bu  + gui + dui) 
# Let's estimate those residuals using the RecommenderLab SVD implementation
...
# transform dataframe into a sparse matrix which is more efficient storing sparse data,
edx_matrix <- sparseMatrix(i = edx$userId,
                           j = edx$movieId,
                           x = residual)
...
# save realMatrix of residuals for visualization in rmarkdown

png('figures/edx_realMatrix.png') 
...
# use Recommenderlab to partition data into a training, test and error sets. 
frequent_users_split <- evaluationScheme(frequent_users, method="split", 
                                         train= 0.9, given=30, goodRating=1.5)
train_set<- getData(frequent_users_split, "train")  
test_set <- getData(frequent_users_split, "known")  
error_set <- getData(frequent_users_split, "unknown")  
...
results$MODEL[7]<-"Recommenderlab SVD Model"
...
#############################################################################
# 
#  FINAL TEST of RECOMMENDERLAB + EFFECTS MODEL' RESIDUALS  on VALIDATION SET
#
##############################################################################
...
# split matrix of frequent users into smaller chunks for processing by Recommenderlab
...
# split matrix of infrequent users into smaller chunks for processing by Recommenderlab
...
# predictions on the validation set using: Yui = mu + reg_bi + reg_bu  + gui + dui + residual
predictions<- validation %>%
  mutate(date = as_datetime(timestamp),
         week=round_date(date, "week")) %>%
  left_join(reg_bis, by="movieId") %>%
...
results$MODEL[9]<-"VALIDATION test: Recommenderlab SVD Model"
